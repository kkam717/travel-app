-- Allow followers to see and search private itineraries from people they follow
-- Non-followers: only public itineraries
-- Followers: public + private itineraries from followed authors

-- 1. Update RLS: allow viewing private itineraries when user follows the author
...
-- 2. Update search_itineraries RPC: include private itineraries when current user follows the author
  ...
  matching AS (
    ...
    WHERE i.forked_from_itinerary_id IS NULL
      AND (
        ...
        OR (i.visibility = 'private' AND EXISTS (
          SELECT 1 FROM follows f WHERE f.follower_id = auth.uid() AND f.following_id = i.author_id
        ))
