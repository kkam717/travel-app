-- RPC to search itineraries by title, destination, or stop names
...
AS $$
  ...
  matching AS (
    ...
    WHERE i.visibility = 'public'
      ...
      AND (
        e.pattern IS NULL
        OR i.title ILIKE e.pattern ESCAPE '\'
        OR i.destination ILIKE e.pattern ESCAPE '\'
        OR EXISTS (
          SELECT 1 FROM itinerary_stops s
          WHERE s.itinerary_id = i.id AND s.name ILIKE e.pattern ESCAPE '\'
        )
      )
  )
  SELECT
